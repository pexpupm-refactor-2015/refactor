...
New::New() : 
  m_title(), m_body(),
  m_entities(), m_entities_reserved(),
  m_more_frequent()
{
  ;
}
...
void New::setReservedWords(const std::string& path)
{
  ...
  while (!f.eof()) {
    f >> aux;
    m_entities_reserved.push_back(aux);
  }
}
...
void New::addEntity(const std::string& entity_name)
{
  ...
  for (std::list<std::string>::iterator i = m_entities_reserved.begin();
       i != m_entities_reserved.end(); i++) {
